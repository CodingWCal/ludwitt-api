...
app.post('/v1/apps/:app_id/events', requireDevKey, async (req, res) => {
  ...
  await recordEvent(app_id, { event, user_id, session_id, metadata, sandbox: req.developer.sandbox });
...
app.get('/v1/admin/cohorts/:cohort_id/snapshots/:date', async (req, res) => {
  ...
  const rows = await exportSnapshot();
...
export { app };
export { _resetForTests } from './store.js';
